...
public class ModEnchantmentMenu extends AbstractContainerMenu {
    ...
    public ModEnchantmentMenu(int pContainerId, Inventory pPlayerInventory, FriendlyByteBuf friendlyByteBuf) {
    ...
    public ModEnchantmentMenu(int pContainerId, Inventory pPlayerInventory, ContainerLevelAccess pAccess) {
        super(ModMenuTypes.MOD_ENCHANTMENT_MENU.get(), pContainerId);
        this.access = pAccess;
        this.addSlot(new Slot(this.enchantSlots, 0, 15, 47) {
            ...
            public boolean mayPlace(ItemStack p_39508_) {
                return p_39508_.is(Items.CROSSBOW) ||
                        p_39508_.is(Items.BOW) ||
                        p_39508_.is(Items.IRON_AXE) ||
                        p_39508_.is(Items.IRON_BOOTS) ||
                        p_39508_.is(Items.IRON_CHESTPLATE) ||
                        p_39508_.is(Items.IRON_HELMET) ||
                        p_39508_.is(Items.IRON_HOE) ||
                        p_39508_.is(Items.IRON_LEGGINGS) ||
                        p_39508_.is(Items.IRON_PICKAXE) ||
                        p_39508_.is(Items.IRON_SHOVEL) ||
                        p_39508_.is(Items.IRON_SWORD);
            }
        ...
        this.addSlot(new Slot(this.enchantSlots, 1, 35, 47) {
            ...
            public boolean mayPlace(ItemStack p_39517_) {
                return p_39517_.is(Items.DIAMOND);
            }
        });
        ...
        if (pInventory == this.enchantSlots) {
            ...
            if (!itemstack.isEmpty() && itemstack.isEnchantable()) {
                this.access.execute((p_39485_, p_39486_) -> {
                    ...
                    for(int k = 0; k < 3; ++k) {
                        this.costs[k] = EnchantmentHelper.getEnchantmentCost(this.random, maxEnchantNum, enchantPowerBonus, itemstack);
                        this.enchantClue[k] = -1;
                        this.levelClue[k] = -1;
                        if (this.costs[k] < k + 1) {
                            this.costs[k] = 0;
                        }
                        else if ((k == 1 && j < 8) || (k == 2 && j < 15)) {
                            this.costs[k] = 0;
                        }
                        this.costs[k] = net.minecraftforge.event.ForgeEventFactory.onEnchantmentLevelSet(p_39485_, p_39486_, k, enchantPowerBonus, itemstack, costs[k]);
                    }
    ...
    public boolean clickMenuButton(Player pPlayer, int pId) {
        if (pId >= 0 && pId < this.costs.length) {
            ...
            if ((itemstack1.isEmpty() || itemstack1.getCount() < i) && !pPlayer.getAbilities().instabuild) {
                return false;
            } else if (this.costs[pId] <= 0 || itemstack.isEmpty()) {
                return false;
            } else {
                this.access.execute((p_39481_, p_39482_) -> {
                    ...
                    if (!list.isEmpty()) {
                        pPlayer.onEnchantmentPerformed(itemstack, 0);
